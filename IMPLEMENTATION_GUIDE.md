...
4. **AI 解读模块**：**OpenAI 兼容 API (如 DeepSeek-V4)**   - 负责对筛选出的英文/中文摘要进行阅读，提炼核心贡献，并针对大学生群体生成通俗易懂的知识补充。
...
- **模型**：项目已全面升级至最新的 **DeepSeek-V4-Pro**。
- **配置**：`base_url` 使用 `https://api.deepseek.com`，模型名称指定为 `deepseek-v4-pro`。
- **优势**：DeepSeek-V4 在数学推理和学术论文解读方面具有极强的性能，且 API 格式与 OpenAI 完全兼容，切换成本极低。
